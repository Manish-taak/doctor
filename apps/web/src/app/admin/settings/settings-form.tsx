...
import { updateSettings, type PlatformSettings } from "@/lib/api/settings"
...
export function AdminSettingsForm({ settings }: { settings: PlatformSettings }) {
  ...
  const handleSaveGeneral = async () => {
    ...
    try {
      await updateSettings(session.accessToken, { platformName, supportEmail })
          ...
          <Card className="ring-foreground/5">
            <CardContent className="flex flex-col gap-5">
              ...
              <div className="flex items-center justify-between gap-4">
                ...
                <Select value={sessionTimeout} onValueChange={(value) => setSessionTimeout(value ?? "30")}>
                ...
                {integrations.map((integration) => (
                  <div key={integration.id} className="flex items-center justify-between gap-4 py-3.5 first:pt-0 last:pb-0">
                    ...
                    <Switch
                      ...
                      onCheckedChange={() =>
                        toast(`Connecting ${integration.name} isn't available in this preview.`)
                      }
                    />
                  </div>
                ))}
